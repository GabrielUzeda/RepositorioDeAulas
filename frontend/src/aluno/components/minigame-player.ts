...
export class MinigamePlayer {
  constructor(activityData, onComplete, senhaCurso = '', senhaAtividade = '') {
    ...
    try {
      const data =
        typeof activityData.json_data === 'string'
      ...
      if (data.questions) {
        this.questions = data.questions.map((q) => {
          const correctOpt =
            (q.options || []).find((o) => o.correct || o.isCorrect) ||
            (q.options ? q.options[0] : null);
          return {
        ...
        this.questions = data.perguntas.map((p) => ({
          ...
          respostaCorreta:
            p.respostaCorreta || p.correta || (p.alternativas ? p.alternativas[0] : ''),
  ...
  update(dtFactor: number = 1) {
    ...
    if (!this.currentEnemy) {
      ...
      const dist = Math.hypot(
        this.player.x - this.currentEnemy.x,
        this.player.y - this.currentEnemy.y
      );
  ...
  draw() {
    ...
    if (!this.controlsLocked) {
      ...
      this.ctx.lineTo(0, 40 + Math.random() * 15);
  ...
  spawnEnemy() {
    ...
    if (qBox) {
      qBox.classList.remove(
        ...
        'translate-y-[100px]',
        '!border-[#ffaa00]',
        '!shadow-[0_0_25px_rgba(255,170,0,0.5)]',
        '!border-red-600',
    ...
    if (errBanner) {
      errBanner.className =
        'hidden bg-[#ffaa00] text-black font-black text-sm uppercase p-2 animate-pulse';
      errBanner.innerText = '';
    }
  ...
  answerChosen(selected, correct, btnElement) {
    ...
    const isCorrect =
      correct !== '' &&
      (selected === correct || selected.trim().toLowerCase() === correct.trim().toLowerCase());
  ...
  handleMistake(btnElement) {
    ...
    if (this.enemyMistakes === 1 && this.currentEnemy) {
      ...
      if (banner) {
        ...
        banner.className =
          'bg-[#ffaa00] text-black font-black text-sm uppercase p-2 text-center animate-pulse block';
      }
      ...
      if (banner) {
        ...
        banner.className =
          'bg-red-600 text-white font-black text-sm uppercase p-2 text-center animate-pulse block';
      }
      document.querySelectorAll('#mg-options-area button').forEach((b) => {
  ...
  showRanking() {
    ...
    modal.className =
      'absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/95 animate-fade-in';
    modal.innerHTML = `
    ...
    fetch(`/api/ranking/${this.activityData.id}`)
      ...
      .catch((err) => {
        ...
        if (list)
          list.innerHTML =
            '<div class="text-center text-red-500 mt-32">Erro ao carregar ranking.</div>';
      });
  }
}
